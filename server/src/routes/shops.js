...
const { syncShopResources, normalizeShopDomain } = require('../services/shopifyService');
...
router.post('/register', async (req, res) => {
  ...
  const domainAliases = [normalizedDomain, rawLower];
  ...
  const shop = await Shop.findOneAndUpdate(
    { shopDomain: { $in: domainAliases } },
    {
      ...
      apiVersion: apiVersion || process.env.SHOPIFY_API_VERSION,
      scopes,
      status: 'active',
    },
